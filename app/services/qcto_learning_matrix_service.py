...
def _estimate_km_ranges(km_modules, job_id=None):
    ...
    def _fetch(module):
        ...
        if cached:
            return cached
        try:
            return generate_qcto_knowledge_module_content(module, job_id=job_id)
        except Exception:
            return {"topics": []}

    all_content = parallel_map(km_modules, _fetch, max_workers=3)

    for module, content in zip(km_modules, all_content):
        if content is None:
            content = {"topics": []}
...
def _estimate_pm_ranges(pm_modules, job_id=None):
    ...
    def _fetch(module):
        ...
        if cached:
            return cached
        try:
            return generate_qcto_practical_module_content(module, job_id=job_id)
        except Exception:
            return {"units": []}

    all_content = parallel_map(pm_modules, _fetch, max_workers=3)

    for module, content in zip(pm_modules, all_content):
        if content is None:
            content = {"units": []}
...
def _estimate_wm_ranges(wm_modules, job_id=None):
    ...
    def _fetch(module):
        ...
        if cached:
            return cached
        try:
            return generate_qcto_workplace_module_content(module, job_id=job_id)
        except Exception:
            return {"units": []}

    all_content = parallel_map(wm_modules, _fetch, max_workers=3)

    for module, content in zip(wm_modules, all_content):
        if content is None:
            content = {"units": []}
...
def build_qcto_learning_matrix_docx(title: str, syllabus_content: dict, organization_name: str = None,
    ...
    _add_branded_header_footer(doc, logo_bytes=logo_bytes, qualification_name=qualification_title, organization_name=organization_name, primary_hex=primary_hex, accreditation_info=accreditation_info, document_label="Learning Matrix")
