...
from app.services.ai_service import reason_isa_traceability, generate_km_exam_objective_questions, generate_km_short_answer_questions
...
def _add_short_answer_section(doc, km_modules, primary, primary_hex, job_id=None):
    ...
    for module in km_modules:
        for topic in module.get("topics", []):
            if not (topic.get("assessment_criteria") or topic.get("elements")):
                continue
            content = generate_km_short_answer_questions(topic, job_id=job_id)
            for q in content.get("questions", []):
                ...
                blank_lines = min(q.get("blank_lines", 4), 8)
        ...
        for topic in module.get("topics", []):
            ...
            for pm_code in link_info.get("pm", []):
                ...
                for pa_raw in pm_module.get("performance_assessment", []):
                    ...
                    scenario_count += 1
                    scenario_text = (
                        f"You have been tasked with applying your practical skills in a "
                        f"real workplace context. {pa_item}. Describe, step by step, how "
                        f"you would carry this out."
                    ).bold = True
