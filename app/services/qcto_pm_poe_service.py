...
def build_qcto_pm_poe_docx(title: str, syllabus_content: dict, organization_name: str = None,
    ...
    # (shared with PM Assessment Guide, so both documents show identical scenarios) —
    # only fetch fresh for modules that don't have cached content yet.
    per_module_results = []
    tasks_needing_fetch = []
    task_positions = []
    for m_idx, module in enumerate(pm_modules):
        ...
        per_module_results.append(None)
        for group in groups:
    ...
    _add_branded_header_footer(doc, logo_bytes=logo_bytes, qualification_name=qualification_title, organization_name=organization_name, primary_hex=primary_hex, accreditation_info=accreditation_info, document_label="PM Portfolio of Evidence")
