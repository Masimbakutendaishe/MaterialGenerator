...
from app.services.ai_service import generate_qcto_knowledge_module_content, parallel_map
...
def build_qcto_knowledge_module_docx(title: str, syllabus_content: dict, organization_name: str = None,
    ...
    # Per-module content
    # Pre-fetch every module's content concurrently, before rendering starts — python-docx
    # itself isn't thread-safe, so the actual document writing below still happens
    # sequentially, but the slow part (waiting on AI calls) no longer happens one module
    # at a time.
    # Use already-cached module content if the task layer generated it (shared with the
    # Learning Matrix, which needs this same content just to estimate page counts) — only
    # fetch fresh for modules that don't have cached content yet.
    module_contents = [None] * len(km_modules)
    modules_needing_fetch = []
    fetch_positions = []
    for m_idx, module in enumerate(km_modules):
    ...
    if modules_needing_fetch:
        fetched = parallel_map(
            modules_needing_fetch,
            lambda m: generate_qcto_knowledge_module_content(m, job_id=job_id),
            max_workers=3,
        )
    ...
    for m_index, module in enumerate(km_modules, start=1):
        ...
        if content is None:
            # This module's content generation failed even after the provider chain was
            # exhausted — say so plainly rather than crashing the whole document.
            content = {
    ...
    _add_branded_header_footer(
        ...
        accreditation_info=accreditation_info, document_label="KM Learner Guide",
    )
