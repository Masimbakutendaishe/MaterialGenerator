...
from app.services.ai_service import generate_pm_scenario_and_questions
...
def _add_pag_toc(doc, primary, primary_hex, module_count):
    ...
    for i in range(1, module_count + 1):
        entries.append(f"About This Practical Skills Module {i}")
        entries.append(f"Scenario and Practical Applied Activities — Module {i}")
    entries += ["Assessment Feedback", "Remediation: Assessment Feedback", "Declaration by Learner"]
...
def _add_pag_scenario_and_activities(doc, module, module_index, primary, primary_hex, secondary, job_id=None):
    """Scenario followed by PA-coded practical activities and IAC-coded assessment
    criteria — a genuine, AI-generated, South African-contextualized scenario with real
    scenario-embedded questions per skill, not a generic placeholder scenario and bare
    'perform a demonstration of X' instructions."""
    ...
    if pa_items:
        content = generate_pm_scenario_and_questions(module, job_id=job_id)

        doc.add_paragraph(
            "The learner is to use the following scenario when completing all the Practical "
        ...
        for i, item in enumerate(content.get("items", []), start=1):
            ...
            doc.add_paragraph(f"Skill: {item.get('pa_text', '')}").runs[0].bold = True

            doc.add_paragraph("Answer the following, in the context of the scenario above:").runs[0].italic = True
            for q_num, question in enumerate(item.get("questions", []), start=1):
            ...
            if iac_items:
                ...
                for j, criterion in enumerate(iac_items, start=1):
                    doc.add_paragraph(f"IAC{i:02d}{j:02d} {criterion}", style="List Bullet")

            _add_observation_sheet(doc, primary, primary_hex, f"{module_code}-PS{i:02d}: {item.get('pa_text', '')}")
    else:
        doc.add_paragraph(
...
def build_qcto_pm_assessment_guide_docx(title: str, syllabus_content: dict, organization_name: str = None,
    ...
    for i, module in enumerate(pm_modules, start=1):
        ...
        _add_pag_scenario_and_activities(doc, module, i, primary, primary_hex, secondary, job_id=job_id)
