...
def _build_pm_module_deck(module, qualification_title, organization_name, brand_colors, logo_bytes=None, job_id=None):
    ...
    for pa_raw in module.get("performance_assessment", []):
        pa_item = pa_raw.get("text", "") if isinstance(pa_raw, dict) else pa_raw
        result = generate_slide_content(pa_item, module_criteria, course_title=qualification_title, job_id=job_id)
        for slide_data in result.get("slides", []):
            slide_title = slide_data.get("title", pa_item)
