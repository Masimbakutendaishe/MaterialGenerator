...
from app.services.ai_service import generate_facilitator_guide_content, generate_pm_facilitation_steps
...
def _add_pm_fg_toc(doc, primary, primary_hex, day_count):
    ...
    for d in range(1, day_count + 1):
        entries.append(f"Facilitation Plan — Day {d}")
    entries.append("Formative Assessment Guide Marking Memorandum")
...
def _add_pm_fg_facilitation_plan(doc, module, day_number, primary, primary_hex, secondary, job_id=None):
    ...
    template phrase with the skill text substituted in."""
    ...
    if units:
        facilitation_content = generate_pm_facilitation_steps(module, job_id=job_id)
...
def build_qcto_pm_facilitator_guide_docx(title: str, syllabus_content: dict, organization_name: str = None,
    ...
    for day_number, module in enumerate(pm_modules, start=1):
        _add_pm_fg_facilitation_plan(doc, module, day_number, primary, primary_hex, secondary, job_id=job_id)
